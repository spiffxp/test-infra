...
import (
	...
	tgconfig_pb "github.com/GoogleCloudPlatform/testgrid/pb/config"
)
...
var testgridProtoPath = flag.String("testgrid-proto", "", "Path to testgrid proto generated by testgrid/cmd/configurator")
...
// Loaded at TestMain.
var prowConfig *prowconfig.Config
var testgridConfig *tgconfig_pb.Configuration
var dashboardTabListByJobName map[string][]*tgconfig_pb.DashboardTab
...
func main() {
	...
	if *jobConfigPath == "" {
		fmt.Println("--job-config must be set")
		os.Exit(1)
	}
	if *testgridProtoPath == "" {
		fmt.Println("--testgrid-proto must be set")
		os.Exit(1)
	}

	prowConfig, err = prowconfig.Load(*configPath, *jobConfigPath)
	if err != nil {
	...
	jobsByJobName := allStaticJobsByName(prowConfig)

	testgridConfig, err = tgconfig.Read(*testgridProtoPath, context.Background(), nil)
	if err != nil {
		fmt.Printf("Could not load config: %v\n", err)
		os.Exit(1)
	}
	dashboardTabListByJobName = make(map[string][]*tgconfig_pb.DashboardTab)
	for _, dashboard := range testgridConfig.Dashboards {
		for _, tab := range dashboard.DashboardTab {
			if job, ok := jobsByJobName[tab.TestGroupName]; ok {
				tabs := dashboardTabListByJobName[job.Name]
				tabs = append(tabs, tab)
				dashboardTabListByJobName[job.Name] = tabs
			} else {
				fmt.Printf("testgrid dashboard tab %s/%s testgroup %s has no matching prowjob\n", dashboard.Name, tab.Name, tab.TestGroupName)
			}
		}
	}
...
// NOTE: this does not account for multiple presubmits/postsubmits sharing a name
//			 (e.g. per-release-branch jobs); which one wins is arbitrary
func allStaticJobsByName(c *prowconfig.Config) map[string]prowconfig.JobBase {
	jobs := make(map[string]prowconfig.JobBase)
	for _, job := range c.AllStaticPresubmits(nil) {
		jobs[job.Name] = job.JobBase
	}
...
func sortedPeriodics() []prowconfig.Periodic {
...
func sortedPresubmitsByRepo() (repos []string, jobsByRepo map[string][]prowconfig.Presubmit) {
	jobsByRepo = make(map[string][]prowconfig.Presubmit)
...
func sortedPostsubmitsByRepo() (repos []string, jobsByRepo map[string][]prowconfig.Postsubmit) {
	jobsByRepo = make(map[string][]prowconfig.Postsubmit)
...
func PrimaryDashboard(job prowconfig.JobBase) string {
...
func OwnerDashboard(job prowconfig.JobBase) string {
	dashboardsAnnotation, ok := job.Annotations["testgrid-dashboards"]
	if !ok {
		// technically it could be specified in a testgrid config, would need testgrid/cmd/configurator code to know for sure
		return "TODO"
...
func isMergeBlocking(job prowconfig.Presubmit) bool {
	return !job.Optional && !job.SkipReport && (job.AlwaysRun || job.RunIfChanged != "")
}

func guessPeriodicRepoAndBranch(job prowconfig.Periodic) (repo, branch string) {
	repo = "TODO"
	branch = "TODO"
...
func NewProwJobReportRow(date, jobType, repo, branch string, alwaysRun, mergeBlocking bool, job prowconfig.JobBase) ProwJobReportRow {
